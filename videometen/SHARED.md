...
| [`src/_reusable/InteractiveChart.tsx`](src/_reusable/InteractiveChart.tsx) | data | React-wrapper rond Chart.js (line + scatter) met playhead-, raaklijn-, meet-lijn-plugins, klik/hover events, wheel/pinch zoom + pan, gedimde punten en theme-aware kleuren. Volledig domein-agnostisch | modelleren (na React-migratie), toekomstige grafische tools | **actief — eerste klant videometen** |
| [`src/_reusable/chart-plugins/playhead.ts`](src/_reusable/chart-plugins/playhead.ts) | data | Chart.js plugin: verticale stippellijn op een data-x-waarde (timeline-playhead) | Modelleren, toekomstige time-series-tools | actief — eerste klant videometen |
| [`src/_reusable/chart-plugins/tangent.ts`](src/_reusable/chart-plugins/tangent.ts) | data | Chart.js plugin: label-pill met `dy/dx = …` rond het midden van een (extern als dataset getekende) raaklijn | Modelleren (vervangt bestaande tangentLabelPlugin), toekomstige analyse-tools | actief — eerste klant videometen |
| [`src/_reusable/chart-plugins/measureLines.ts`](src/_reusable/chart-plugins/measureLines.ts) | data | Chart.js plugin: twee verticale meet-lijnen (cyan + amber). Handles zelf worden door de wrapper als absolute divs gerenderd | Modelleren (vervangt bestaande measureLinesPlugin), toekomstige analyse-tools | actief — eerste klant videometen |
| [`src/_reusable/niceAxis.ts`](src/_reusable/niceAxis.ts) | data | Snap as-bereik [lo, hi] naar nette ronde stappen (1/2/5·10ⁿ), optioneel pinMin + padTop. Geport uit modelleren | Modelleren, elke chart-tool met handmatige as-rendering | kandidaat — wacht op tweede gebruiker |
| [`src/_reusable/useThemeColors.ts`](src/_reusable/useThemeColors.ts) | ui | Theme-aware hook die NH-design-token CSS-vars (`--accent`, `--text-muted`, etc.) als concrete strings teruggeeft. Updates bij `data-theme`-wissel via MutationObserver — handig in canvas-renderers | Alle tools die buiten-CSS kleuren nodig hebben (Chart.js, SVG-renderers, etc.) | kandidaat — wacht op tweede gebruiker |
...
| [`src/_reusable/csvNL.ts`](src/_reusable/csvNL.ts) | data | Excel-NL-vriendelijke CSV-generatie: `;` als separator, `,` als decimaal, UTF-8 BOM, automatische quoting van strings met `;`/`"`/newline. Plus `downloadBlob`-helper voor browser-downloads | Elke tool met tabular export naar Excel-NL | kandidaat — wacht op tweede gebruiker |
